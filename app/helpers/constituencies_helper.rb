...
def karte
  wahlkreis = JSON.parse(Net::HTTP.get('v1.d-api.de', "/parlament.bund.wahlkreise.geodaten?output_type=json&limit=1&id=#{@constituency.number}"))

  geodata = wahlkreis['data'][0]['koordinaten']
  @name = wahlkreis['data'][0]['name']

  geodata.collect! {|point|
    point.collect! {|coord|
      coord.to_f
    }
    [point[1],point[0]]
  ...
  encoder = GMapPolylineEncoder.new()
  result = encoder.encode(geodata)

  return "http://maps.google.com/maps/api/staticmap?size=400x400&sensor=false&path=fillcolor:0xAA000033|color:0xFFFFFF00|enc:#{result[:points]}"
